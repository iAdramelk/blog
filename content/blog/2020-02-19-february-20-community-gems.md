...
[Git-like experience](<(https://dvc.org/doc/command-reference/checkout)>),
meaning that it will sync your local workspace with all the model files,
dependencies, and outputs specified by a project's .dvc files. If you only want
to access one artifact from the project, you can do this with
`dvc checkout <path to .dvc file>`. This will attempt to find the object
...
environment- things you wouldn't want to push to a GitHub repo. DVC only
modifies `config.local` when you explicitly use the `--local` flag in the
`dvc config` or `dvc remote *` commands, so outside of these cases you shouldn't
...
### Q: I have an Azure storage account containre, and the only link I can see in my Azure portal for the container is an `http://` link. But the tutorial on DVC shows Azure storage accessed with the `azure://` protocol. [Which is right?](https://discordapp.com/channels/485586884165107732/563406153334128681/675087897661276169)
...
Generally we don't recommend sharing access credentials- there are other ways to
grant access to data files in your DVC project, like setting up `https://`
access to your data repository so users can use `dvc import` and `dvc get` to
grab specific files (see the last question in
[last month's Community Gems](https://blog.dvc.org/january-20-community-gems)).

With that caveat, for Google Drive, `gdrive_client_id` and
`gdrive_client_secret` aren't used to access a specific user's Google Drive
disk; they're predominantly used by Google's API to
...
it with your credentials. Whether this risk is acceptable is up to you.

### Q: I just tried to upgrade DVC via `homebrew` and got a hash problem. [What's going on](https://discordapp.com/channels/485586884165107732/485596304961962003/672930535261339669)?
...
DVC is now part a Homebrew default! Please uninstall and reinstall using
`brew install dvc` for uninterrupted upgrades in the future.

### Q: I still can't convince myself to version-control the data rather than meta-data. Can anyone give me a strong argument against version controlling data file paths in config files instead of using DVC?
...
To summarise, DVC is not the only way to version your data. But we think it's
