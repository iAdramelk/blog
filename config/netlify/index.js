...
function Wrapper({widgetFor}) { return widgetFor('body'); }
...
CMS.registerEditorComponent({
  ...
  pattern :
      /<ExternalLink(\s+[a-zA-Z]+\s*=\s*("([^"]*)"|'([^']*)'))\s(\s+[a-zA-Z]+\s*=\s*("([^"]*)"|'([^']*)'))\s(\s+[a-zA-Z]+\s*=\s*("([^"]*)"|'([^']*)'))\s(\s+[a-zA-Z]+\s*=\s*("([^"]*)"|'([^']*)'))*\s*\/>/,
  ...
  toPreview : function(obj) {
    return (
        `<a key="${obj.href}" href="${obj.href}" class="external-link-preview">
