...
exports.onCreateNode = ({node, actions, getNode}) => {
  ...
  if (node.internal.type === 'MarkdownRemark') {
    ...
    if (descriptionLong) {
      const descriptionLong_value =
          remark().use(remarkHTML).processSync(descriptionLong).toString();
      createNodeField(
          {name : 'descriptionLong', node, value : descriptionLong_value});
    }

    const pictureComment = node.frontmatter.pictureComment;
    if (pictureComment) {
      const pictureComment_value =
          remark().use(remarkHTML).processSync(pictureComment).toString();
      createNodeField(
          {name : 'pictureComment', node, value : pictureComment_value});
    }
  }
};
